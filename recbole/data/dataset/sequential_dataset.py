...
class SequentialDataset(Dataset):
    ...
    def data_augmentation(self):
        ...
        for field in self.inter_feat:
            if field != self.uid_field:
                ...
                if list_field == 'item_id_list' and self.config['model'] == 'OverRec':
    ...
    def no_data_augmentation(self):
        ...
        last_uid = None
        uid_list, item_list_index, target_index, item_list_length = [], [], [], []
        ori_uid_list, ori_item_list_index, ori_target_index, ori_item_list_length = [], [], [], []
        seq_start = 0
        for i, uid in enumerate(self.inter_feat[self.uid_field].numpy()):
            if last_uid != uid:
                last_uid = uid
                seq_start = i
                if i != 0:
                    ori_uid_list += uid_list[-3:]
                    ori_item_list_index += item_list_index[-3:]
                    ori_target_index += target_index[-3:]
                    ori_item_list_length += item_list_length[-3:]
            else:
                if i - seq_start > max_item_list_len:
                    seq_start += 1
        ...
        uid_list = np.array(ori_uid_list)
        item_list_index = np.array(ori_item_list_index)
        target_index = np.array(ori_target_index)
        item_list_length = np.array(ori_item_list_length, dtype=np.int64)
