...
from recbole.utils import ensure_dir, get_local_time, early_stopping, calculate_valid_score, dict2str, \
    EvaluatorType, KGDataLoaderState, get_tensorboard, set_color, get_gpu_usage, get_my_tensorboard
...
class Trainer(AbstractTrainer):
    ...
    def __init__(self, config, model):
        ...
        self.tensorboard = get_my_tensorboard(config)
    ...
    def _add_train_loss_to_tensorboard(self, epoch_idx, losses, tag='Loss/Train'):
        if isinstance(losses, tuple):
            for idx, loss in enumerate(losses):
                self.tensorboard.add_scalar(tag + str(idx), loss/self.config['train_batch_size'], epoch_idx)
        else:
            self.tensorboard.add_scalar(tag, losses/self.config['train_batch_size'], epoch_idx)
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                for i in valid_result:
                    self.tensorboard.add_scalar('Vaild_score/' + i, valid_result[i], epoch_idx)
