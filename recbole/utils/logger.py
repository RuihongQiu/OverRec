...
def init_logger(config):
    ...
    LOGROOT += config['dataset'] + '/{}-aug-{}/{}-dp0-bs{}-lr{}-ly{}-{}/'.format(
        config['model'], config['aug'], config['hidden_size'], config['train_batch_size'], config['learning_rate'], config['num_layers'], get_local_time())
    dir_name = os.path.dirname(LOGROOT)
    ensure_dir(dir_name)
